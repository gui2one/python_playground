...
if __name__ == "__main__":
    ...
    kernelString = "__kernel void matrix_dot_vector(__global const float4 *matrix,"
    kernelString += " __global const float4 *vector, __global float *result)"
    kernelString += "{"
    kernelString += "int gid = get_global_id(0);"
    kernelString += "result[gid] = dot(matrix[gid], vector[0]);"
    kernelString += "}"
    ...
    destination_buf = cl.Buffer(
        context, mem_flags.WRITE_ONLY, matrix_dot_vector.nbytes)

    ## Step #9. Associate the arguments to the kernel with kernel object.
    ## Step #10. Deploy the kernel for device execution.
    program.matrix_dot_vector(
        queue, matrix_dot_vector.shape, None, matrix_buf, vector_buf, destination_buf)
